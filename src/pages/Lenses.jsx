...
import { Eye, SlidersHorizontal, RotateCcw, ChevronLeft, ChevronRight, Filter } from 'lucide-react';
import ProductCard from '../components/ProductCard';
import SearchBar from '../components/SearchBar';
import '../styles/cameras.css';

const lensSubCategories = [
  "All", 
  "Prime Lenses", 
  "Zoom Lenses", 
  "Wide-Angle Lenses", 
...
const Lenses = () => {
  ...
  useEffect(() => {
    ...
    if (sub) {
      const match = lensSubCategories.find(c => c.toLowerCase() === sub.toLowerCase() || c.toLowerCase().includes(sub.toLowerCase()));
  ...
  const lensProducts = useMemo(() => {
    return allProducts.filter(p => p.category?.toLowerCase() === 'lenses');
  }, [allProducts]);

  const brandsList = useMemo(() => {
    return ['All', ...new Set(lensProducts.map(p => p.brand).filter(Boolean))];
  }, [lensProducts]);
    ...
    return lensProducts
      .filter(item => {
        ...
        const matchesSubCat = selectedSubCat === 'All' || 
          itemSub === targetSub ||
          itemSub.replace(/ lenses$/i, '') === targetSub.replace(/ lenses$/i, '');
  ...
  }, [lensProducts, searchTerm, selectedSubCat, selectedBrand, priceRange, sortBy]);

  const totalPages = Math.max(1, Math.ceil(filteredLenses.length / itemsPerPage));
  const paginatedLenses = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return filteredLenses.slice(start, start + itemsPerPage);
  }, [filteredLenses, currentPage]);
    ...
    <div className="category-page-container">
      ...
      <div className="category-hero-header">
        <div className="container category-hero-inner">
          <div className="hero-cat-tag"><Eye size={14} /> MASTER GLASS & OPTICS</div>
          <h1 className="category-page-title">Interchangeable Camera Lenses</h1>
          <p className="category-page-desc">
            Ultra-fast primes, versatile standard zooms, telephoto wildlife glass, and macro optics engineered for Sony E, Canon RF, Nikon Z, and Fujifilm X mount systems.
          </p>

          {/* Subcategory Pills */}
          <div className="subcategory-pills-row">
            {lensSubCategories.map(sub => (
              <button
                key={sub}
      ...
      <div className="container category-main-layout">
        ...
        <div className="mobile-filter-bar">
          ...
          >
            <span><Filter size={16} /> Filter & Sort Lenses</span>
            <span>({filteredLenses.length} Items)</span>
          </button>
        </div>
        ...
        <aside className={`filters-sidebar ${mobileFilterOpen ? 'mobile-show' : ''}`}>
          ...
          <div className="filter-group">
            <h4 className="filter-group-title">Lens Brand</h4>
        ...
        <main className="category-products-area">
          <div className="catalog-toolbar">
            <SearchBar
              ...
              placeholder="Search lenses by focal length, mount..."
            />

            <div className="catalog-toolbar-right">
              <span className="items-count-text">
                Showing <strong>{paginatedLenses.length}</strong> of <strong>{filteredLenses.length}</strong> lenses
              </span>
          ...
          {filteredLenses.length === 0 ? (
            <div className="empty-catalog-state">
              ...
              <h3>No Lenses Match Your Filter</h3>
              <p>Try clearing your active subcategory or brand selection.</p>
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap', marginTop: '16px' }}>
                {selectedSubCat !== 'All' && (
                  <button onClick={() => setSelectedSubCat('All')} className="btn-secondary">
                    Show All {selectedBrand !== 'All' ? selectedBrand : ''} Lenses
                  </button>
                )}
                {selectedBrand !== 'All' && (
