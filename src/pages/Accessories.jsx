...
import { BatteryCharging, SlidersHorizontal, RotateCcw, ChevronLeft, ChevronRight, Filter } from 'lucide-react';
...
const Accessories = () => {
  ...
  useEffect(() => {
    ...
    if (sub) {
      const match = accessorySubCategories.find(c => c.toLowerCase() === sub.toLowerCase());
  ...
  const brandsList = useMemo(() => {
    return ['All', ...new Set(accessoryProducts.map(p => p.brand).filter(Boolean))];
  }, [accessoryProducts]);
    ...
    return accessoryProducts
      .filter(item => {
        ...
        const matchesSubCat = selectedSubCat === 'All' || 
          itemSub === targetSub || 
          itemSub.replace(/s$/, '') === targetSub.replace(/s$/, '');
  ...
  }, [accessoryProducts, searchTerm, selectedSubCat, selectedBrand, priceRange, sortBy]);

  const totalPages = Math.max(1, Math.ceil(filteredAccessories.length / itemsPerPage));
  const paginatedAccessories = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return filteredAccessories.slice(start, start + itemsPerPage);
    ...
    <div className="category-page-container">
      <div className="category-hero-header">
        <div className="container category-hero-inner">
          <div className="hero-cat-tag"><BatteryCharging size={14} /> PHOTOGRAPHY ESSENTIALS</div>
          <h1 className="category-page-title">Camera Accessories, Filters & Media</h1>
          <p className="category-page-desc">
            V90 ultra-speed cinema SD & CFexpress cards, high-density genuine batteries, variable ND filters, cleaning kits, and ergonomic camera straps.
          </p>
      ...
      <div className="container category-main-layout">
        <div className="mobile-filter-bar">
          ...
          >
            <span><Filter size={16} /> Filter & Sort Accessories</span>
            <span>({filteredAccessories.length} Items)</span>
          </button>
        </div>
        ...
        <main className="category-products-area">
          <div className="catalog-toolbar">
            <SearchBar
              ...
              placeholder="Search SD cards, batteries, ND filters..."
            />

            <div className="catalog-toolbar-right">
              <span className="items-count-text">
                Showing <strong>{paginatedAccessories.length}</strong> of <strong>{filteredAccessories.length}</strong> products
              </span>
          ...
          {filteredAccessories.length === 0 ? (
            <div className="empty-catalog-state">
              <BatteryCharging size={48} color="var(--primary)" />
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap', marginTop: '16px' }}>
                {selectedSubCat !== 'All' && (
                  <button onClick={() => setSelectedSubCat('All')} className="btn-secondary">
                    Show All {selectedBrand !== 'All' ? selectedBrand : ''} Accessories
                  </button>
                )}
                {selectedBrand !== 'All' && (
            ...
            <div className="product-grid">
              {paginatedAccessories.map(product => (
