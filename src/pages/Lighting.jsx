...
import { SunMedium, SlidersHorizontal, RotateCcw, ChevronLeft, ChevronRight, Filter } from 'lucide-react';
...
const Lighting = () => {
  ...
  useEffect(() => {
    ...
    if (sub) {
      const match = lightingSubCategories.find(c => c.toLowerCase() === sub.toLowerCase() || c.toLowerCase().includes(sub.toLowerCase()));
  ...
  const lightingProducts = useMemo(() => {
    return allProducts.filter(p => p.category?.toLowerCase() === 'lighting');
  }, [allProducts]);

  const brandsList = useMemo(() => {
    return ['All', ...new Set(lightingProducts.map(p => p.brand).filter(Boolean))];
  }, [lightingProducts]);
    ...
    return lightingProducts
      .filter(item => {
        ...
        const matchesSubCat = selectedSubCat === 'All' || 
          itemSub === targetSub ||
          itemSub.replace(/ (lights|units)$/i, '') === targetSub.replace(/ (lights|units)$/i, '');
  ...
  }, [lightingProducts, searchTerm, selectedSubCat, selectedBrand, priceRange, sortBy]);

  const totalPages = Math.max(1, Math.ceil(filteredLighting.length / itemsPerPage));
  const paginatedLighting = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return filteredLighting.slice(start, start + itemsPerPage);
  }, [filteredLighting, currentPage]);
    ...
    <div className="category-page-container">
      <div className="category-hero-header">
        <div className="container category-hero-inner">
          <div className="hero-cat-tag"><SunMedium size={14} /> STUDIO & ON-LOCATION LIGHTING</div>
          <h1 className="category-page-title">Photography & Video Studio Lighting</h1>
          <p className="category-page-desc">
            Equip your creative space with daylight balanced LED panels, high-output studio flash strobes, parabolic softboxes, and bi-color ring lights.
          </p>
      ...
      <div className="container category-main-layout">
        <div className="mobile-filter-bar">
          ...
          >
            <span><Filter size={16} /> Filter & Sort Lighting</span>
            <span>({filteredLighting.length} Items)</span>
          </button>
        </div>
        ...
        <aside className={`filters-sidebar ${mobileFilterOpen ? 'mobile-show' : ''}`}>
          ...
          <div className="filter-group">
            <h4 className="filter-group-title">Lighting Brand</h4>
        ...
        <main className="category-products-area">
          <div className="catalog-toolbar">
            <SearchBar
              ...
              placeholder="Search studio lights, strobes..."
            />

            <div className="catalog-toolbar-right">
              <span className="items-count-text">
                Showing <strong>{paginatedLighting.length}</strong> of <strong>{filteredLighting.length}</strong> products
              </span>
          ...
          {filteredLighting.length === 0 ? (
            <div className="empty-catalog-state">
              <SunMedium size={48} color="var(--primary)" />
              <h3>No Lighting Gear Matched</h3>
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap', marginTop: '16px' }}>
                {selectedSubCat !== 'All' && (
                  <button onClick={() => setSelectedSubCat('All')} className="btn-secondary">
                    Show All {selectedBrand !== 'All' ? selectedBrand : ''} Lighting Gear
                  </button>
                )}
                {selectedBrand !== 'All' && (
